...
class SyncService {
  ...
  // Start periodic sync checking - optimized for better performance
  startPeriodicSync() {
    // Immediate sync check on startup (after a short delay)
    setTimeout(() => {
      if (this.isOnline && !this.syncInProgress) {
        const pendingActivities = getActivitiesByStatus('local');
        if (pendingActivities.length > 0) {
          console.log('🔄 Starting initial sync check - found', pendingActivities.length, 'pending activities');
          this.performBatchSync('startup');
        }
      }
    }, 2000);
