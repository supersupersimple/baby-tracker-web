...
export const LIMITS = {
  ...
  MAX_TOTAL_ACTIVITIES_PER_BABY: parseInt(process.env.MAX_TOTAL_ACTIVITIES_PER_BABY) || 10000,
};
...
export async function isAccountCreationAllowed(prisma) {
  const userCount = await prisma.user.count();
  return userCount < LIMITS.MAX_ACCOUNTS;
}

// Utility function to check if user can create more babies
export async function canUserCreateBaby(prisma, userId) {
  const babyCount = await prisma.baby.count({
    where: { ownerId: userId }
  });
  return babyCount < LIMITS.MAX_BABIES_PER_USER;
}

// Utility function to check if baby can be shared with more users
export async function canShareBaby(prisma, babyId) {
  const shareCount = await prisma.babyAccess.count({
    where: { 
      babyId: babyId,
      role: { not: 'OWNER' } // Don't count owner
    }
  });
  return shareCount < LIMITS.MAX_SHARED_USERS_PER_BABY;
}

// Utility function to check if user can create more activities today
export async function canCreateActivityToday(prisma, babyId) {
  ...
  return activityCount < LIMITS.MAX_ACTIVITIES_PER_DAY;
}

// Utility function to check if baby can have more total activities
export async function canCreateActivity(prisma, babyId) {
  const activityCount = await prisma.activity.count({
    where: { babyId: babyId }
  });
  return activityCount < LIMITS.MAX_TOTAL_ACTIVITIES_PER_BABY;
}
