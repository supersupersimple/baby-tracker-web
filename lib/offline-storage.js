...
export const storeActivityLocally = (activityData) => {
  try {
    ...
    // Check for potential duplicates within 5-minute window
    const isDuplicate = checkForDuplicate(localData.activities, contentHash, activityData.fromDate);
    if (isDuplicate) {
      console.warn('Potential duplicate activity detected, skipping store');
      return isDuplicate;
    }
    ...
    console.log(`💾 Storing new activity locally:`, {
...
const checkForDuplicate = (activities, contentHash, fromDate, timeWindowMs = 5 * 60 * 1000) => {
  ...
  try {
    ...
    if (activityToRemove) {
      ...
      const updatedQueue = syncQueue.filter(
        queueActivity => queueActivity.id !== activityId && queueActivity.tempId !== activityId
      );
  ...
  try {
    ...
    for (const remoteActivity of remoteActivities) {
      ...
      const existingActivity = localData.activities.find(localActivity => {
        // 1. Direct server ID match (most reliable)
        if (localActivity.serverId === remoteActivity.id) {
          return true;
        }
        
        // 2. Content hash match (for activities created from same source)
        if (localActivity.contentHash === generateContentHash(remoteActivity)) {
          return true;
        }
        
        // 3. Exact match for very recent activities (within 5 minutes) to handle sync race conditions
        if (localActivity.babyId === remoteActivity.babyId &&
            localActivity.type === remoteActivity.type &&
            localActivity.subtype === remoteActivity.subtype) {
          const localTime = new Date(localActivity.fromDate).getTime();
          const remoteTime = new Date(remoteActivity.fromDate).getTime();
          const timeDiff = Math.abs(localTime - remoteTime);
          
          // If the activities are within 5 seconds of each other, consider them the same
          if (timeDiff < 5000) {
            console.log(`🔗 Found matching activity by time proximity: local ${localActivity.id} <-> remote ${remoteActivity.id}`);
            return true;
          }
        }
