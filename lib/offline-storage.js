...
const generateContentHash = (record) => {
  try {
    ...
    const content = {
      ...
      timestamp: record.fromDate ? new Date(record.fromDate).getTime() : Date.now()
    };
    // Simple hash function for content comparison - use UTF-8 safe encoding
    const contentString = JSON.stringify(content);
    
    // Use TextEncoder for UTF-8 safe encoding instead of btoa
    if (typeof TextEncoder !== 'undefined') {
      ...
      // Fallback for environments without TextEncoder - sanitize string first
      const sanitized = contentString.replace(/[^\x00-\x7F]/g, ""); // Remove non-ASCII
      return btoa(sanitized).replace(/[^a-zA-Z0-9]/g, '').substring(0, 16);
    }
  } catch (error) {
    console.error('Error generating content hash:', error, record);
...
export const storeActivityLocally = (activityData) => {
  try {
    ...
    const activityWithId = {
      ...
      // Add user info if present (for creator differentiation)
...
// Clear all local data for a specific baby
export const clearBabyLocalData = (babyId) => {
  try {
    const localData = getLocalData();
    
    // Remove all activities for this baby
    const originalCount = localData.activities.length;
    localData.activities = localData.activities.filter(
      activity => activity.babyId !== babyId
    );
    
    // Remove from sync queue for this baby
    localData.syncQueue = localData.syncQueue.filter(
      activity => activity.babyId !== babyId
    );
    ...
    const removedCount = originalCount - localData.activities.length;
    console.log(`Cleared ${removedCount} local activities for baby ${babyId}`);
    return { success: true, removedCount };
  } catch (error) {
    console.error('Error clearing baby local data:', error);
