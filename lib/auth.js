...
// Create Drizzle adapter with explicit table mapping
...
// Create wrapped adapter with error handling
const wrappedAdapter = {
  ...drizzleAdapter,
  async getUserByAccount(accountData) {
    try {
      return await drizzleAdapter.getUserByAccount(accountData);
    } catch (error) {
...
export const authConfig = {
  ...
  callbacks: {
    ...
    async signIn({ user: authUser }) {
      try {
        console.log(`🔐 Sign-in attempt for: ${authUser.email}`);
        
        // Check if account creation is allowed (max accounts limit)
        const existingUser = await db.select().from(users).where(eq(users.email, authUser.email)).get();
        
        if (existingUser) {
          console.log(`✅ Existing user signing in: ${authUser.email}`);
          return true;
        }
        
        // New user - check if we can create new accounts
        console.log(`🆕 New user attempting to sign up: ${authUser.email}`);
        const canCreateAccount = await isAccountCreationAllowed(db);
        if (!canCreateAccount) {
          console.log(`❌ Account creation blocked: Max accounts limit reached. Email: ${authUser.email}`);
          return false; // Deny sign in
        }
        
        console.log(`✅ New user account creation allowed: ${authUser.email}`);
        return true; // Allow sign in
      } catch (error) {
        console.error(`❌ Error in signIn callback for ${authUser.email}:`, error);
        return false;
      }
    },
